...
const section = z.discriminatedUnion('type', [
  z.object({ type: z.literal('hero'), variant: z.enum(['feature', 'banner']).optional().default('feature'), eyebrow: optionalText, title: z.string(), subtitle: optionalText, text: optionalText, image: imagePath.optional().nullable(), imageAlt: optionalText, image2: imagePath.optional().nullable().transform((v) => v || undefined), image2Alt: optionalText, primary: cta, secondary: cta, showTrust: z.boolean().optional().default(true) }),
  ...
  z.object({ type: z.literal('services'), ...titled, body: optionalText, cta }),
  ...
  z.object({ type: z.literal('faq'), ...titled, category: z.enum(['all', 'general', 'sugaring', 'laser', 'booking', 'men']).optional().default('all'), limit: z.number().int().optional(), items: z.array(z.object({ question: z.string(), answer: z.string() })).optional().default([]), image: imagePath.optional().nullable().transform((v) => v || undefined), imageAlt: optionalText }),
  ...
  z.object({ type: z.literal('contact'), ...titled, showForm: z.boolean().optional().default(true), image: imagePath.optional().nullable().transform((v) => v || undefined), imageAlt: optionalText, cards: z.boolean().optional().default(false), mapLink: z.boolean().optional().default(false) }),
  z.object({ type: z.literal('blogList'), ...titled, limit: z.number().int().optional(), posts: z.array(z.string()).optional().default([]) }),
  z.object({ type: z.literal('visit'), ...titled, image: imagePath.optional().nullable().transform((v) => v || undefined), imageAlt: optionalText }),
  z.object({ type: z.literal('why'), ...titled, body: optionalText, image: imagePath, imageAlt: z.string(), imageSide: z.enum(['left', 'right']).optional().default('left'), counter: z.object({ value: z.number(), suffix: optionalText, label: z.string() }).optional().nullable().transform((v) => v || undefined), items: z.array(z.string()).optional().default([]), cta }),
  z.object({ type: z.literal('ticker'), words: z.array(z.string()).min(1), tone: z.enum(['dark', 'light']).optional().default('dark') }),
  z.object({ type: z.literal('cta'), ...titled, cta, image: imagePath.optional().nullable().transform((v) => v || undefined), imageAlt: optionalText }),
  z.object({ type: z.literal('checklist'), ...titled, tint: z.boolean().optional().default(false), groups: z.array(z.object({ title: optionalText, items: z.array(z.string()).min(1) })).min(1) }),
  z.object({ type: z.literal('offerTable'), ...titled, tables: z.array(z.object({ title: z.string(), column: optionalText, treatwellServiceId: treatwellId, rows: z.array(z.object({ name: z.string(), price: z.number() })).min(1) })).min(1), cta }),
